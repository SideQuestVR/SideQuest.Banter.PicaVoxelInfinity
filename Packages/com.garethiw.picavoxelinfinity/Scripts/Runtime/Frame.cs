...
//         public int ZSize = 32;
//
//         // IsDirty is set whenever updates to a frame are made via one of the SetVoxel methods. PicaVoxel does not set IsDirty back again
//         // This is for use in your game, so you can check it to see if changes have been made (for saving to disk etc.). Remember to set it back yourself!
...
// #if UNITY_EDITOR
//                 if (!Application.isPlaying)
//                     UnityEditor.EditorUtility.DisplayProgressBar("PicaVoxel", $"Creating Chunks ({ParentVolume.name})", (1f / (float)(XSize * YSize * ZSize)) * (float)(x * y * z));
// #endif
//             }
//
// #if UNITY_EDITOR
...
//                 {
//                     if (!Application.isPlaying)
//                         UnityEditor.EditorUtility.DisplayProgressBar("PicaVoxel", $"Saving Chunks ({ParentVolume.name})",
